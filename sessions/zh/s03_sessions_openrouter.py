...
用法:
    cd claw0
    python sessions/zh/s03_sessions_openrouter.py
...
class SessionStore:
    ...
    def _rebuild_history(self, path: Path) -> list[dict]:
        ...
        OpenAI API 规则:
          ...
          - tool_use 转换为 assistant 消息的 tool_calls 数组
        ...
        for line in lines:
            ...
            if rtype == "user":
                ...
                tool_call = {
                    ...
                    "function": {
                        "name": record["name"],
                        "arguments": json.dumps(record["input"], ensure_ascii=False),
                    }
                }
                if messages and messages[-1]["role"] == "assistant" and "tool_calls" not in messages[-1]:
                    messages[-1]["tool_calls"] = [tool_call]
                elif messages and messages[-1]["role"] == "assistant" and "tool_calls" in messages[-1]:
...
class ContextGuard:
    ...
    def estimate_tokens(text: str) -> int:
        # 一个非常粗略的估算: 1 token ~ 4 chars (适用于英文, 中文可能更密集)
        return len(text) // 4
    ...
    def compact_history(self, messages: list[dict],
        ...
        keep_count = max(4, int(total * 0.2)) # 保留最后 20% 的消息, 最少保留4条
        compress_count = max(2, int(total * 0.5)) # 压缩前 50% 的消息
        compress_count = min(compress_count, total - keep_count) # 确保不压缩最后要保留的消息
    ...
    while True:
        ...
        while True:
            ...
            if finish_reason == "stop":
                ...
                store.save_turn("assistant", assistant_content)
